...
def normalize_values_q99(values, params):
    """
    Normalize values using percentile (Q1/Q99) robust normalization to [-1, 1] range.

    Maps 98% of the data (1st~99th percentile) to [-1, 1] and clips outliers.

    Args:
        values: Input values, shape (T, D) or (B, T, D)
        params: Dictionary with "q01" and "q99" keys, each shape (D,) or (T, D)

    Returns:
        Normalized values clipped to [-1, 1], same shape as input.
    """
    q01_vals = params["q01"]
    q99_vals = params["q99"]

    normalized = np.zeros_like(values, dtype=float)
    mask = ~np.isclose(q99_vals, q01_vals)

    # [q01, q99] -> [0, 1] -> [-1, 1]
    normalized[..., mask] = (values[..., mask] - q01_vals[..., mask]) / (
        q99_vals[..., mask] - q01_vals[..., mask]
    )
    normalized[..., mask] = 2 * normalized[..., mask] - 1
...
def unnormalize_values_q99(normalized_values, params):
    """
    Q99 unnormalization from [-1, 1] range back to original range.
    Args:
        normalized_values: Normalized input values in [-1, 1] range
            - Shape: (T, D) or (B, T, D)
        params: Dictionary with "q01" and "q99" keys
            - params["q01"]: 1st percentile values used for normalization
            - params["q99"]: 99th percentile values used for normalization
    Returns:
        Unnormalized values in original range [q01, q99]
            - Same shape as input normalized_values: (T, D) or (B, T, D)
            - Input values are clipped to [-1, 1] before unnormalization
    """
    q01_vals = params["q01"]
    q99_vals = params["q99"]
    range_vals = q99_vals - q01_vals

    # [-1, 1] -> [0, 1] -> [q01, q99]
    unnormalized = (np.clip(normalized_values, -1.0, 1.0) + 1.0) / 2.0 * range_vals + q01_vals
...
def parse_modality_configs(
    ...
    """Parse modality configs into a flat ``{modality: ModalityConfig}`` map.

    Accepts both formats:
    - Flat: ``{"state": ModalityConfig(...), "action": ...}``
    - Legacy nested: ``{"new_embodiment": {"state": ..., "action": ...}}``
    """
    if not modality_configs:
        return {}

    first_value = next(iter(modality_configs.values()))
    is_legacy_nested = isinstance(first_value, dict) and "delta_indices" not in first_value

    source: dict[str, Any]
    if is_legacy_nested:
        # Pick the canonical PAS embodiment entry when present, otherwise the first one.
        source = modality_configs.get("new_embodiment", first_value)
    else:
        source = modality_configs

    parsed_modality_configs: dict[str, ModalityConfig] = {}
    for modality, config in source.items():
        if isinstance(config, dict):
            parsed_modality_configs[modality] = ModalityConfig(**config)
        else:
            parsed_modality_configs[modality] = config
